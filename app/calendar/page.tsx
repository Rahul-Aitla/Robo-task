            ...
            <div className="flex-1 flex flex-col min-w-0 h-screen overflow-y-auto">
                <header className="border-b border-gray-200 bg-white/80 backdrop-blur sticky top-0 z-40 px-6 py-3 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-slate-500">Content Calendar</span>
                    </div>
                </header>
                                    ...
                                    {days.map((day: Date, dayIdx: number) => {
                                        ...
                                        return (
                                            <div
                                                key={day.toString()}
                                                className={clsx(
                                                    "bg-white min-h-[120px] p-2 hover:bg-gray-50 transition-colors relative group",
                                                    !isSameMonth(day, currentDate) && "bg-gray-50/50 text-gray-400"
                                                )}
                                            >
                                                <time dateTime={format(day, 'yyyy-MM-dd')} className={clsx(
                                                    "text-xs font-medium block mb-1",
                                                    isSameDay(day, new Date()) ? "bg-indigo-600 text-white w-6 h-6 rounded-full flex items-center justify-center" : "text-gray-700"
                                                )}>
                                                    {format(day, 'd')}
                                                </time>
                                                <div className="space-y-1">
                                                    {dayEvents.map((event) => (
                                                        <div
                                                            ...
                                                            className="text-[10px] p-1.5 rounded border bg-white shadow-sm hover:shadow-md transition-all cursor-pointer truncate"
                                                        >
